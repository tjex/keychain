...
takelock() {
	...
	while [ "$counter" -lt "$(($lockwait * 2))" ]; do
		lockfile && return 0
		sleep 0.5
		counter=$(($counter + 1))
	done
	...
	for pm_k in "$@"; do
		...
		if wantagent ssh; then
			if [ -f "$pm_k" ]; then
				...
				add_sshkey "$HOME/.ssh/$pm_k"
				continue
			elif [ -f "$HOME/.ssh2/$pm_k" ]; then
				add_sshkey "$HOME/.ssh2/$pm_k"
				continue
...
mesg "${PURP}Note:${OFF} This is a private fork of: https://funtoo.org/Funtoo:Keychain"
[ "$myaction" = version ] && {
	versinfo
	exit 0
}
...
if wantagent gpg; then
	...
	GPG_TTY=$(tty)
	export GPG_TTY # fall back to ncurses pinentry
